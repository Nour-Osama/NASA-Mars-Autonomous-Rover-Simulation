...
def update_yaw_rate(Rover):
    # this correction assumes rover can't do extreme turns very fast
    # it is valid when rover velocity isn't too high 
    # correcting when previous in 4th quarter and current in first quarter
    cond = Rover.previous_yaw >=270 and Rover.yaw <= 90
    previous_yaw = (360 - Rover.previous_yaw) if (cond) else Rover.previous_yaw
    # correcting when current in 4th quarter and previous in first quarter
    cond = Rover.yaw >=270 and Rover.previous_yaw <= 90
    current_yaw = (360 - Rover.yaw) if (cond) else Rover.yaw
    # calculate yaw_rate
    yaw_rate = abs((current_yaw-previous_yaw) / (Rover.total_time-Rover.previous_time))
...
def update_rover(Rover, data):
    ...
    # update yaw_rate, previous_Time and previous_yaw 
